...
def process_vars(df, var):

    for i in range(len(var)): # f has chunk, on, field
        f = var[i]
        curr = df[f.chunk]
        for c in range(len(curr)):
            if df.loc[c,f.chunk].field == f.field:
                df.loc[c,f.chunk] = f
    return df
...
def make_fields(df):
    ...
    df['cnt_f_float_opp'] = np.logical_or(np.logical_or(df.action_p == 'C' , np.logical_and(np.logical_and(np.logical_and(np.logical_and(df.action_p == 'CC', df.flg_p_face_raise ), df.flg_f_open_opp ), len(df.str_aggressors_p) == 2 ), df.str_aggressors_p[1] > df.position) ), df.flg_f_has_position)
    ...
    df.head().to_csv('X:/Users/Josh/Desktop/POKER PROJECT/StageH - recreating cash_cache/Fields/temp_complete.csv')
...
def make_df(field_df,db,name,summary_path):
    ...
    newpath = 'X:/Users/Josh/Desktop/POKER PROJECT/StageH - recreating cash_cache/' + name
    if not os.path.exists(newpath): os.makedirs(newpath)
    os.chdir(newpath)
...
add_field('flg_f_has_position',9)
...
for db in range(start,finish+1):
    ...
    # CREATE THE DATAFRAME OF FIELDS
