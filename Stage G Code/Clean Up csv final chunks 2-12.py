...
def import_csv(db, batch):
    path = 'X:/Users/Josh/Desktop/POKER PROJECT/StageF - Dump Tables/cash_hand_player_statistics/' + str(db) + '/16_batch' + str(batch) + '_' + str(db) + '.csv'
...
def hand_no(db):
    ...
    hn['hand_no'].to_csv('X:/Users/Josh/Desktop/POKER PROJECT/StageG/hand_no_' + str(db) + '.csv')
...
def clean_db(db, batchrange):
    ...
    for batch in batchrange:
        ...
        if batch == 1:
            ...
            summary_df = df[['id_hand','player_name','description','hand_no']] # a summary df of the db
...
unique = {}
for db in range(9,15): # what else do i want on summary df? 3. amt_expected_won, 9. amt_before
# for db in range(1,2):
    # clean_db(db, [i for i in range(1,13)])# if i in (1, 11, 12)]) # loop
    # clean_db(db, [1,10,11,12]) # loop
    # unique[db] =  clean_db(db, [i for i in range(1,13)]) # loop
    clean_db(db, [i for i in range(1,13)]) # loop
    # print db, len(unique[db])
    # uniques = DataFrame.from_dict(unique)
    # uniques.to_csv('X:/Users/Josh/Desktop/POKER PROJECT/StageG/hand_numers.csv')

    # clean_df.head()

# process the summaries and db 1
# add the hand num
