...
def get_valid_hand(db):
    ...
    # print df.valid
    # df.to_csv(path) # run this through once and the above won't be needed... this causes problems
...
def hand_no(db):
    hn = read_csv('X:/Users/Josh/Desktop/POKER PROJECT/StageF - Dump Tables/Tables/15_cash_hand_summary_'
        + str(db) + '.csv')
    print 'Cash Hand Summary: Length   Max' + str(len(hn)) + '   ' + str(hn.id_hand.min()) + '    ' + str(hn.id_hand.max())
...
def clean_db(db, batchrange):
    ...
    for batch in batchrange:
        ...
        if batch == 1:
            print 'Start: DB Batch Length' + '   '  + str(db) + '   ' + str(batch) + '   ' +  str(len(df))
            # create id_hand
            id_list = get_id_hand(df) # return hand_num_list
            df['id_hand'] = id_list

            print 'id_list :' + str(len(id_list))
            print 'Hands: ' + str(len(set(df.id_hand.values)))
            print 'Min/Max: ' + str(df.id_hand.min()) + '  /   ' + str(df.id_hand.max())
            ...
            print 'Summary: ' + str(len(summary_df))
            summary_df.to_csv('X:/Users/Josh/Desktop/POKER PROJECT/StageG/db' + str(db) + '_SUMMARY.csv')
            print 'Final: DB Batch Length' + '   '  + str(db) + '   SUMMARY   ' +  str(len(df))
            # print summary_df.columns
            # print 'len ' + str(summary_df.columns)
            ...
            if 'id_hand' not in df.columns:
                # create id_hand
                print 'should be the same: ' + str(len(df))
        ...
        print 'Final DB Hands: ' + str(len(set(df.id_hand.values)))
        ...
        print 'Final: DB Batch Length' + '   '  + str(db) + '   '  + str(batch) + '   '  +  str(len(df))
...
for db in range(7,13): # what else do i want on summary df? 3. amt_expected_won, 9. amt_before
# for db in range(1,2):
